...
static void vhost_user_blk_start(VirtIODevice *vdev)
{
    ...
    /* guest notifiers has been initialized, invalid the previous
     * vhost_virtqueue's masked_notifier to backend first,
     * vhost-user-blk will setup the right guest notifier at last.
     */
    for (i = 0; i < s->dev.nvqs; i++) {
        struct vhost_vring_file file;
        file.index = s->dev.vhost_ops->vhost_get_vq_index(&s->dev, i);
        file.fd = -1;
        ret = s->dev.vhost_ops->vhost_set_vring_call(&s->dev, &file);
        if (ret < 0) {
            error_report("vhost_set_vring_call failed: %d", -ret);
...
static Property vhost_user_blk_properties[] = {
    ...
    DEFINE_PROP_UINT32("max-segment-size", VHostUserBlk, max_segment_size,
                       131072),
    DEFINE_PROP_UINT32("max-segment-num", VHostUserBlk, max_segment_num, 128),
