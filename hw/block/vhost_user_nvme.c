...
static int nvme_set_guest_notifier(NvmeCtrl *n)
{
    int ret;
    NvmeCQueue *cq;
    uint32_t qid;
...
static uint16_t nvme_doorbell_buffer_config(NvmeCtrl *n, NvmeCmd *cmd)
{
    ...
    /* enable guest notifier here as the slave target
     * can only use shadow doorbell buffer
     * for I/O processing
     */
    if (nvme_set_guest_notifier(n)) {
        error_report("nvme_doorbell_buffer_config: set guest notifier failed");
        return NVME_INVALID_FIELD | NVME_DNR;
    }
...
static int nvme_clear_ctrl(NvmeCtrl *n, bool shutdown)
{
    ...
    if (n->dataplane_started && shutdown) {
        nvme_clear_guest_notifier(n);
    }
