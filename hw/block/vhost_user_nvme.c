...
static void nvme_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    if (vhost_dev_nvme_init(&n->dev, &n->chardev,
    ...
    if (n->shadow_bar_size > 4096 && (n->shadow_bar_size % 8 == 0)) {
        n->shadow_db = NULL;
        n->shadow_bar = qemu_memfd_alloc("shadow-bar0", n->shadow_bar_size,
                                     F_SEAL_GROW | F_SEAL_SHRINK | F_SEAL_SEAL,
                                     &n->shadow_bar_fd, errp);
        if (!n->shadow_bar) {
            error_report("vhost-user-nvme: create shared memory fd failed");
            goto err;
	}

        ret = vhost_user_nvme_set_bar(&n->dev, n->shadow_bar,
                                      n->shadow_bar_size,
                                      n->shadow_bar_fd);
        if (ret < 0) {
        ...
        n->shadow_db = (volatile uint32_t *)((uintptr_t)n->shadow_bar + 0x1000ull);
    ...
    info_report("Emulated Controller Capabilities 0x%"PRIx64"",
            n->bar.cap);
...
err_clean:
    qemu_memfd_free(n->shadow_bar, n->shadow_bar_size, n->shadow_bar_fd);
err:
    nvme_cleanup(n);
...
static Property nvme_props[] = {
    ...
    DEFINE_PROP_UINT32("bar_size", NvmeCtrl, shadow_bar_size, 8192),
