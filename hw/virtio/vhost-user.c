...
int vhost_user_nvme_set_bar(struct vhost_dev *dev, void *bar,
                            uint32_t size, int fd)
{
    ...
    msg.payload.memory.regions[0].userspace_addr = (uintptr_t)bar;
    msg.payload.memory.regions[0].memory_size = size;
    msg.payload.memory.regions[0].guest_phys_addr = 0;
