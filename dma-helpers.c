...
static void dma_blk_list_cb(void *opaque, int ret)
{
    DMAAIOCB *dbs = (DMAAIOCB *)opaque;
    dma_addr_t cur_addr, cur_len;
    dma_addr_t offset_size = dbs->offset_size;
    void *mem;
    ...
    dbs->acb = NULL;
    dbs->offset = dbs->offset_list[dbs->cur_offset_index];
     ...
     * We're adding entries to iovec until we get offset_size amount of data
     */
    do {
                            ...
                            void *opaque, DMADirection dir)
{
    ...
    trace_dma_blk_io(dbs, io_func_opaque, offset_list[0],
...
{
    return dma_blk_io_list(blk_get_aio_context(blk), sg, offset_list, offset_size, align,
                           dma_blk_read_io_func, blk, cb, opaque,
                           DMA_DIRECTION_FROM_DEVICE);
}
